...
app = Flask(__name__)
# CRITICAL FIX: Use environment variable for secret key (fixes login persistence after Render restart)
app.secret_key = os.environ.get('SECRET_KEY', 'dev-key-change-in-production-phishguard-2026')

# Database configuration - CRITICAL FIX: Use persistent storage for Render
# Default to current directory for local dev, Render will set DATABASE_DIR env var
...
if DATABASE_DIR != '.' and not os.path.exists(DATABASE_DIR):
    try:
        ...
        print(f"⚠️  Cannot create {DATABASE_DIR}, using current directory. Error: {e}")
        DATABASE_DIR = '.'
