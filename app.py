...
app = Flask(__name__)
app.secret_key = os.environ.get('SECRET_KEY', 'dev-key-change-in-production-phishguard-2026')

# ============================================================
# DATABASE CONFIGURATION
# ============================================================
# Render FREE tier: filesystem is EPHEMERAL — SQLite DB is wiped
# every time the service sleeps (after 15 min inactivity).
#
# Solution: If DATABASE_URL is set (PostgreSQL), use that.
...
if USE_POSTGRES:
    ...
    print(f"📊 Using PostgreSQL (data persists across sleep!)")
else:
    ...
    print(f"📊 Using SQLite: {DATABASE}  (local dev only — will NOT persist on Render free)")


class DictRow:
    """Wrapper to make psycopg2 rows behave like sqlite3.Row (supports [] and dict())"""
...
class PgCursorWrapper:
    """Makes a psycopg2 cursor behave like a sqlite3 cursor:
       - Translates ? → %s
       - Returns DictRow objects so dict(row) and row['col'] both work
    """
...
class PgConnectionWrapper:
    """Makes a psycopg2 connection behave like a sqlite3 connection:
       - conn.execute() works directly
       - cursor() returns PgCursorWrapper
    """
...
def migrate_db():
    """Add Uzbek columns to exam_questions if they don't exist yet.
       Safe to call on every startup. Handles both SQLite and PostgreSQL."""
    if USE_POSTGRES:
        ...
        try:
            ...
            for col in uz_cols:
                if col not in existing_cols:
                    cur.execute(f"ALTER TABLE exam_questions ADD COLUMN {col} TEXT")
            conn.commit()
        except:
            pass
        finally:
...
def init_db():
    """Initialize database with tables — works with both SQLite and PostgreSQL"""
    conn = get_db()
    cursor = conn.cursor()
    ...
    # CRITICAL FIX: Ensure admin user exists with correct password
    # This handles both creation and password reset scenarios
    ...
    if not admin_user:
        ...
        # Update existing admin to ensure password is correct (fixes login issues)
    ...
    if cursor.fetchone()['count'] == 0:
        ...
        # FIXED: Professional Uzbek translations
        default_questions = [
            ("What is phishing?",
             "A type of fish", "A cyber attack using fake emails", "A computer virus", "A hacking tool", "b",
             "Fishing nima?",
             "Baliq turi", "Soxta elektron pochta orqali amalga oshiriladigan kiberxujum", "Kompyuter virusi", "Xakerlik vositasi"),
            ("Which is a red flag in emails?",
             "Company logo", "Urgent requests for personal info", "Professional grammar", "Known sender", "b",
             "Elektron pochtada xavfli belgi qaysi?",
             "Kompaniya logotipi", "Shaxsiy ma'lumotlarni tezda talab qilish", "Professional grammatika", "Tanish jo'natuvchi"),
            ("What should you do with suspicious emails?",
             ...
             "Shubhali elektron pochta bilan nima qilish kerak?",
             "Havolalarni bosish va tekshirish", "Ma'lumotlaringiz bilan javob berish", "IT/Xavfsizlik bo'limiga xabar berish", "Do'stlarga yuborish"),
            ("What is spear phishing?",
             "Fishing in the ocean", "Targeted phishing attack", "Mass email campaign", "Antivirus software", "b",
             "Spear fishing (nishonli fishing) nima?",
             "Okeanda baliq ovlash", "Muayyan shaxslarga qaratilgan maqsadli xujum", "Ommaviy elektron pochta kampaniyasi", "Antivirus dasturi"),
            ("Best way to verify suspicious email?",
             "Click the link", "Call sender using known contact", "Reply to email", "Ignore it", "b",
             "Shubhali elektron pochtani tekshirishning eng yaxshi usuli?",
             "Havolani bosish", "Rasmiy telefon raqami orqali jo'natuvchiga qo'ng'iroq qilish", "Elektron pochtaga javob berish", "E'tiborsiz qoldirish"),
            ("What is MFA?",
             "Multiple File Access", "Multi-Factor Authentication", "Main Frame Access", "Manual Form Approval", "b",
             "MFA nima uchun qisqartma?",
             "Ko'p faylga kirish", "Ko'p bosqichli autentifikatsiya (Multi-Factor Authentication)", "Asosiy ramkaga kirish", "Qo'lda forma tasdiqlash"),
            ("Common phishing technique?",
             "Strong passwords", "Fake login pages", "Encrypted emails", "Secure connections", "b",
             "Keng tarqalgan fishing usuli?",
             "Kuchli parollar", "Soxta kirish sahifalarini yaratish", "Shifrlangan elektron pochta", "Xavfsiz ulanishlar"),
            ("What to check in URLs?",
             "Color scheme", "Domain spelling", "Font size", "Image quality", "b",
             "URL manzillarida nimani tekshirish kerak?",
             "Rang sxemasi", "Domen nomining to'g'ri yozilishi", "Shrift o'lchami", "Tasvir sifati"),
            ("Purpose of phishing awareness?",
             "Increase email traffic", "Reduce security incidents", "Slow down work", "Replace antivirus", "b",
             "Fishing xavfidan xabardor qilish maqsadi?",
             "Elektron pochta trafigini oshirish", "Xavfsizlik hodisalarini kamaytirish va ma'lumotlarni himoya qilish", "Ishni sekinlashtirish", "Antivirusni almashtirish"),
            ("What makes a password strong?",
             "Common words", "Personal info", "Mix of characters, numbers, symbols", "Short and simple", "c",
             "Parolni kuchli qiladigan narsa nima?",
             "Oddiy so'zlar", "Shaxsiy ma'lumotlar", "Katta va kichik harflar, raqamlar va belgilar aralashmasi", "Qisqa va sodda")
        ]
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if user_row:
                ...
                print(f"   User found: {user.get('username')}, is_admin: {user.get('is_admin', 0)}")
                ...
                if check_password_hash(user['password_hash'], password):
                    ...
                    print(f"   ➡️  Redirecting to: {'admin panel' if user.get('is_admin') else 'home'}")
...
def register():
    ...
    try:
        ...
        try:
            # Hash password using same method as admin creation
            password_hash = generate_password_hash(password)
            
            # Insert user
            cursor = conn.execute(
                "INSERT INTO users (name, surname, username, password_hash) VALUES (?, ?, ?, ?)",
...
# CRITICAL: This MUST be at module level, NOT inside
# if __name__ == '__main__', because gunicorn imports the
# module but does NOT execute the __main__ block.
# ============================================================
print("=" * 60)
...
try:
    print(f"\n📂 Database setup:")
    print(f"   Directory: {DATABASE_DIR}")
    ...
    print(f"   Directory exists: {os.path.exists(DATABASE_DIR)}")
    import traceback
    traceback.print_exc()
